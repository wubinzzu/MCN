...
from utils.dataset import RecDataset, SRecDataset
from utils.dataloader import STrainDataLoader, TrainDataLoader, SEvalDataLoader, EvalDataLoader
...
def quick_start(model, dataset, config_dict, save_model=True):
    ...
    if config['model_type'] != 'Sequential':
        ...
        train_data = STrainDataLoader(config, train_dataset, batch_size=config['train_batch_size'], shuffle=True)
        (valid_data, test_data) = (
            SEvalDataLoader(config, valid_dataset, additional_dataset=train_dataset,
                           batch_size=config['eval_batch_size']),
            SEvalDataLoader(config, test_dataset, additional_dataset=train_dataset,
                           batch_size=config['eval_batch_size']))
