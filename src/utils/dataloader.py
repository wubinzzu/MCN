...
class STrainDataLoader(TrainDataLoader):
    """
    General dataloader with negative sampling.
    """
    ...
    def __init__(self, config, dataset, batch_size=1, shuffle=False):
        ...
        if self.config['model'] in self.config['seq2seq_models']:
            self.sample_func = self.sasrec_sample
            uid_field = self.dataset.uid_field
            iid_field = self.dataset.iid_field
            ...
            for index, row in dataset.df.iterrows():
                u = row[uid_field]
                item = row[iid_field]
                user_list = row[item_list_field]
                user_list = list(filter(lambda value: value != dataset.item_num, user_list))
                user_list.append(item)
                self.history_items_per_u[u] = set(user_list)

    def sasrec_sample(self):
        cur_data = self.dataset[self.pr: self.pr + self.step]
        self.pr += self.step
        # to tensor
        user_tensor = torch.tensor(cur_data[self.config['USER_ID_FIELD']].values).type(torch.LongTensor).to(self.device)
        ...
        # sequences
        column_tensors = []
        for column in cur_data.columns:
            if column == self.config['USER_ID_FIELD'] or column == self.config['ITEM_ID_FIELD']:
                continue  # 跳过用户 ID 列和物品 ID 列
            column_data = cur_data[column].tolist()
            column_tensor = torch.tensor(column_data).to(self.device)
        ...
        if self.config['model'] in self.config['seq2seq_models']:  # SASRec
            # 对sequences负采样
            neg_item_list = []

            def sample_neg_ids(u_ids):
                neg_ids = []
                for u in u_ids:
                    # random 1 item
                    iid = random.sample(range(0, self.dataset.item_num), 1)[0]
                    while iid in self.history_items_per_u[u]:
                        iid = random.sample(range(0, self.dataset.item_num), 1)[0]
                    neg_ids.append(iid)
                return torch.tensor(neg_ids).type(torch.LongTensor)
            ...
            for index, row in cur_data.iterrows():
                user_id = row[self.config['USER_ID_FIELD']]
                user_item_list = row['item_list_field']
                temp = []
                for num in user_item_list:
                    if num != self.dataset.item_num:
                        temp.append(sample_neg_ids([user_id]))
                    else:
                        temp.append(self.dataset.item_num)
                neg_item_list.append(torch.tensor(temp).to(self.device))
            neg_item_list = torch.stack(neg_item_list).to(self.device)

        return [batch_tensor, column_tensors[0], neg_item_list]
    ...
    def _get_neg_sample(self):
        ...
        #     temp = []
        #     for num in range(neg_samples):
        #         temp.append(self._sample_neg_ids([user_id]))
        #
        #     neg_item_list.append(torch.tensor(temp).to(self.device))
        # neg_item_list = torch.stack(neg_item_list).to(self.device)
        #
        # if self.config['model'] == 'SASRec':  # SASRec
        #     # 对sequences负采样
        #     neg_item_list = []
        #
        #     for index, row in cur_data.iterrows():
        ...
        return [batch_tensor, column_tensors[0]]
        # return [batch_tensor, column_tensors[0], neg_item_list]
...
class SEvalDataLoader(EvalDataLoader):
    """
        additional_dataset: training dataset in evaluation
    """
    ...
    def __init__(self, config, dataset, additional_dataset=None,
                 batch_size=1, shuffle=False):
        super().__init__(config, dataset, additional_dataset,
                         batch_size, shuffle)
        self.item_list_per_u = self._get_item_list_per_u(self.eval_u)
        ...
        if self.config['model'] in self.config['seq2seq_models']:
            ...
            item_list_field = 'item_list_field'
            pos_items_per_u = []
            for index, row in additional_dataset.df.iterrows():
                u = row[uid_field]
                item = row[iid_field]
                u_ids.append(u)
                i_ids.append(item)
                ...
                user_list = list(filter(lambda value: value != dataset.item_num, user_list))
                user_list.append(item)
                pos_items_per_u.append(user_list)
            uu = []
            ii = []
            self.train_pos_len_list = []
            for i, u in enumerate(self.eval_u):
                u_ls = pos_items_per_u[int(u)]
                i_len = len(u_ls)
                self.train_pos_len_list.append(i_len)
                uu.extend([u] * i_len)
                ii.extend(u_ls)
            self.pos_items_per_u = torch.tensor([uu, ii]).type(torch.LongTensor)

    def _get_item_list_per_u(self, eval_users):
        eval_users = np.asarray(eval_users.cpu())
        uid_field = self.dataset.uid_field
        item_list_field = self.config['ITEM_LIST_FIELD']
        # load avail items for all uid
        uid_freq = self.dataset.df.groupby(uid_field)
        self.item_list_per_u = []
        for u in eval_users:
            u_ls = uid_freq.get_group(u)[item_list_field].tolist()
            self.item_list_per_u.append(u_ls[0])
        self.item_list_per_u = np.asarray(self.item_list_per_u)
        return torch.tensor(self.item_list_per_u).type(torch.LongTensor).to(self.device)
    ...
    def _next_batch_data(self):
        ...
        return [batch_users, batch_mask_matrix, self.item_list_per_u[batch_users]]
